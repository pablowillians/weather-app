...
RSpec.describe Services::Infrastructure::Adapters::Weather::GoogleHourlyForecast do
  ...
  describe "#call" do
    context "when zipcode is provided" do
      ...
      let(:cache_key) { "google_hourly_forecast_#{zipcode}" }

      context "when the response is cached" do
        let(:cached_data) { load_weather_fixture("hourly_forecast_success") }

        before do
          allow(cache).to receive(:read).with(cache_key).and_return(cached_data)
        end

        it "returns a GoogleHourlyForecastResponse with :cached_response source" do
          result = described_class.new.call(latitude, longitude, zipcode: zipcode)

          expect(result).to be_a(Services::Infrastructure::Adapters::Weather::GoogleHourlyForecastResponse)
      ...
      context "when the API returns a successful response" do
        let(:success_data) { load_weather_fixture("hourly_forecast_success") }
    ...
    context "when zipcode is not provided" do
      let(:cache_key) { "google_hourly_forecast_#{latitude}_#{longitude}" }
      ...
      context "when the API returns a successful response" do
        ...
        it "returns a GoogleHourlyForecastResponse with :api_response source" do
          result = described_class.new.call(latitude, longitude)
    ...
    context "when the API returns no forecast data" do
      let(:not_found_data) { load_weather_fixture("hourly_forecast_not_found") }
      ...
      it "raises NotFoundError" do
        expect { described_class.new.call(latitude, longitude) }
          .to raise_error(
            ...
            "No hourly forecast data found for coordinates: #{latitude}, #{longitude}"
          )
      end
    end
